# -*- coding: utf-8 -*-
"""Factory for creating chat models - Kimi via Baseten ONLY."""
...
from ..providers import get_active_llm_config, get_chat_model_class, get_provider_chat_model, load_providers_json
...
def _get_formatter_for_chat_model(chat_model_class: Type[ChatModelBase]) -> Type[FormatterBase]:
    return _CHAT_MODEL_FORMATTER_MAP.get(chat_model_class, OpenAIChatFormatter)

def _create_file_block_support_formatter(base_formatter_class: Type[FormatterBase]) -> Type[FormatterBase]:
    class FileBlockSupportFormatter(base_formatter_class):
        async def _format(self, msgs):
            msgs = _sanitize_tool_messages(msgs)
            return await super()._format(msgs)
        @staticmethod
        def convert_tool_result_to_string(output: str | list[dict]) -> tuple[str, Sequence[Tuple[str, dict]]]:
            ...
            try:
                ...
                for block in output:
                    ...
                    if block["type"] == "file":
                        ...
                        textual_output.append(f"The returned file '{file_name}' can be found at: {file_path}")
...
def _create_model_instance(llm_cfg: Optional["ResolvedModelConfig"]) -> Tuple[ChatModelBase, Type[ChatModelBase]]:
    if llm_cfg and llm_cfg.is_local:
        model = create_local_chat_model(model_id=llm_cfg.model, stream=True, generate_kwargs={"max_tokens": None})
...
def _get_chat_model_class_from_provider() -> Type[ChatModelBase]:
    try:
        ...
        if provider_id:
            return get_chat_model_class(get_provider_chat_model(provider_id, providers_data))
    except Exception:
        pass
    return OpenAIChatModel

def _create_remote_model_instance(chat_model_class: Type[ChatModelBase]) -> ChatModelBase:
    """Kimi via Baseten ONLY."""
    ...
    logger.info(f"Creating Kimi model: {model_name}")

    model = chat_model_class(
        model_name,
        api_key=api_key,
        stream=True,
        client_kwargs={"base_url": base_url, "timeout": 120.0},
    )
    return model

def _create_formatter_instance(chat_model_class: Type[ChatModelBase]) -> FormatterBase:
    return _create_file_block_support_formatter(_get_formatter_for_chat_model(chat_model_class))()
